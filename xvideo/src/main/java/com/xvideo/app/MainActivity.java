...
public class MainActivity extends Activity {
    ...
    private static final String FILTER_JS = "javascript:(function(){" +
        "if(window.__xplan10)return;window.__xplan10=1;" +
        "var activeV=null,activeA=null,currentSpeed=1,seeking=false,pending=new Set(),raf=0;" +
        "var st=document.createElement('style');st.textContent=`" +
        "html,body,#react-root,main{margin:0!important;padding:0!important;background:#fff!important;color:#111!important;overscroll-behavior:none!important}body{overflow-x:hidden!important}" +
        ...
        ".xp-play{position:absolute!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%)!important;z-index:2147483646!important;width:72px!important;height:72px!important;border-radius:50%!important;border:1px solid rgba(0,0,0,.08)!important;background:rgba(255,255,255,.94)!important;color:#111!important;font-size:30px!important;display:flex!important;align-items:center!important;justify-content:center!important;box-shadow:0 8px 28px rgba(0,0,0,.15)!important}.xp-play.xp-playing{opacity:.2!important}" +
        "#xp-controls{display:none;position:fixed;left:14px;right:14px;bottom:14px;z-index:2147483647;padding:8px 11px;background:rgba(255,255,255,.94);border:1px solid rgba(0,0,0,.07);border-radius:18px;box-sizing:border-box;box-shadow:0 6px 22px rgba(0,0,0,.10);font-family:sans-serif}#xp-row{display:flex;align-items:center;gap:10px;height:34px}#xp-progress{flex:1;min-width:0;height:30px;accent-color:#111}#xp-time{color:#333;font-size:12px;min-width:78px;text-align:right;white-space:nowrap}" +
        "#xp-actions{display:none;position:fixed;right:16px;bottom:80px;z-index:2147483647;gap:8px}#xp-actions button{height:42px;min-width:48px;padding:0 13px;border:1px solid rgba(0,0,0,.07);border-radius:21px;background:rgba(255,255,255,.94);color:#111;font-size:14px;font-weight:700;box-shadow:0 5px 16px rgba(0,0,0,.10)}" +
        ...
        "function showControls(x){controls.style.display=x?'block':'none';actions.style.display=x?'flex':'none'}" +
        "function syncBtn(v,a){var b=a&&a.querySelector('.xp-play');if(!b)return;b.textContent=v.paused?'▶':'❚❚';b.classList.toggle('xp-playing',!v.paused)}" +
        ...
        "function finishBind(a,m){if(!a.isConnected||a.dataset.xpBound==='1')return;a.dataset.xpBound='1';a.dataset.xpMedia='1';a.style.setProperty('display','block','important');var r=a.getBoundingClientRect();a.style.transform='translateX('+(-r.left)+'px) translateZ(0)';a.style.width=innerWidth+'px';isolate(a,m.el);if(m.type==='video'){prepVideo(m.el,false);try{m.el.pause()}catch(e){}addPlay(a,m.el)}else{m.el.loading='eager';m.el.decoding='async'}io.observe(a);nearIo.observe(a);setupScroller()}" +
        "function classifyVideo(a,v){if(a.dataset.xpClassifying==='1')return;a.dataset.xpClassifying='1';a.style.setProperty('display','none','important');prepVideo(v,false);var done=false;function decide(){if(done)return;var d=v.duration;if(isFinite(d)&&d>0){done=true;a.dataset.xpClassifying='0';if(d<60){a.dataset.xpRejected='1';a.style.setProperty('display','none','important');try{v.pause();v.preload='none'}catch(e){}}else finishBind(a,{type:'video',el:v})}}v.addEventListener('loadedmetadata',decide,{once:true});v.addEventListener('durationchange',decide,{once:true});if(v.readyState>=1)decide();setTimeout(function(){if(!done&&a.isConnected){a.dataset.xpClassifying='0';if(isFinite(v.duration)&&v.duration>0&&v.duration<60){a.dataset.xpRejected='1';a.style.setProperty('display','none','important')}else finishBind(a,{type:'video',el:v})}},7000)}" +
        "function inspect(a,attempt){if(!a||!a.isConnected||a.dataset.xpBound==='1'||a.dataset.xpRejected==='1')return;var m=mediaOf(a);if(m){if(m.type==='video')classifyVideo(a,m.el);else finishBind(a,m);return}if(attempt<5)setTimeout(function(){inspect(a,attempt+1)},650);else a.style.setProperty('display','none','important')}" +
        "function queue(n){if(!n||n.nodeType!==1)return;var a=n.matches&&n.matches('article[data-testid=\\\"tweet\\\"]')?n:(n.closest?n.closest('article[data-testid=\\\"tweet\\\"]'):null);if(a)pending.add(a);if(n.querySelectorAll)n.querySelectorAll('article[data-testid=\\\"tweet\\\"]').forEach(function(x){pending.add(x)});if(!raf)raf=requestAnimationFrame(flush)}" +
        ...
        "var io=new IntersectionObserver(function(es){es.forEach(function(e){var a=e.target,m=mediaOf(a);if(!m)return;if(m.type==='video'){prepVideo(m.el,e.isIntersecting&&e.intersectionRatio>.45);if(e.isIntersecting&&e.intersectionRatio>=.55)setActive(m.el,a);if(e.intersectionRatio<.12&&!m.el.paused)m.el.pause()}else if(e.isIntersecting&&e.intersectionRatio>=.55)setActive(null,a)})},{threshold:[0,.12,.45,.55,.85]});" +
        "var nearIo=new IntersectionObserver(function(es){es.forEach(function(e){if(!e.isIntersecting)return;var m=mediaOf(e.target);if(m&&m.type==='video'){prepVideo(m.el,true);if(m.el.readyState<2){try{m.el.load()}catch(x){}}}else if(m&&m.type==='image'){m.el.loading='eager';m.el.decoding='async'}})},{rootMargin:'110% 0px 110% 0px',threshold:0});" +
        "new MutationObserver(function(ms){ms.forEach(function(mu){mu.addedNodes.forEach(queue);var a=mu.target&&mu.target.closest?mu.target.closest('article[data-testid=\\\"tweet\\\"]'):null;if(a&&a.dataset.xpBound!=='1'&&a.dataset.xpRejected!=='1')pending.add(a)});if(pending.size&&!raf)raf=requestAnimationFrame(flush)}).observe(document.body,{childList:true,subtree:true});" +
        ...
        "var controls=document.createElement('div');controls.id='xp-controls';var row=document.createElement('div');row.id='xp-row';var range=document.createElement('input');range.id='xp-progress';range.type='range';range.min='0';range.max='1000';range.value='0';var time=document.createElement('span');time.id='xp-time';time.textContent='0:00 / 0:00';row.appendChild(range);row.appendChild(time);controls.appendChild(row);document.body.appendChild(controls);" +
        "sound.onclick=function(){if(activeV){activeV.muted=!activeV.muted;sound.textContent=activeV.muted?'🔇':'🔊'}};var speeds=[.5,1,1.5,2],si=1;speed.onclick=function(){si=(si+1)%speeds.length;currentSpeed=speeds[si];speed.textContent=currentSpeed+'x';if(activeV)activeV.playbackRate=currentSpeed};" +
    ...
    private GradientDrawable outlined(int fill,int stroke,int radius){ GradientDrawable d=rounded(fill,radius); d.setStroke(dp(1),stroke); return d; }
    private Button makeButton(String text,int textColor,int bgColor){ Button b=new Button(this); b.setText(text); b.setTextColor(textColor); b.setTextSize(14f); b.setAllCaps(false); b.setGravity(Gravity.CENTER); b.setPadding(dp(10),0,dp(10),0); b.setBackground(rounded(bgColor,14)); return b; }
    ...
    @Override public void onCreate(Bundle b){
        ...
        searchBox=new EditText(this); searchBox.setSingleLine(true); searchBox.setTextColor(Color.rgb(20,20,22)); searchBox.setHintTextColor(Color.rgb(130,130,136)); searchBox.setHint("搜索图片、长视频、用户或话题"); searchBox.setTextSize(15f); searchBox.setImeOptions(EditorInfo.IME_ACTION_SEARCH); searchBox.setPadding(dp(14),0,dp(14),0); searchBox.setBackground(outlined(Color.rgb(247,247,249),Color.rgb(228,228,232),14)); bar.addView(searchBox,new LinearLayout.LayoutParams(0,dp(44),1f));
        ...
        WebSettings s=webView.getSettings(); s.setJavaScriptEnabled(true); s.setDomStorageEnabled(true); s.setMediaPlaybackRequiresUserGesture(true); s.setLoadWithOverviewMode(false); s.setUseWideViewPort(false); s.setTextZoom(100); s.setCacheMode(WebSettings.LOAD_DEFAULT); s.setLoadsImagesAutomatically(true); s.setBlockNetworkImage(false);
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.M) s.setOffscreenPreRaster(true); if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O) s.setSafeBrowsingEnabled(true);
