...
public class MainActivity extends Activity {
    ...
    private static final String FILTER_JS = "javascript:(function(){" +
        "if(window.__xv7)return;window.__xv7=1;" +
        ...
        "html,body,#react-root,main{margin:0!important;padding:0!important;background:#000!important;overscroll-behavior:none!important;}body{overflow-x:hidden!important;}" +
        ...
        "main,main>div,main>div>div,section{width:100%!important;max-width:none!important;margin:0!important;padding:0!important;background:#000!important;}" +
        ...
        ".xv-play{position:absolute!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%)!important;z-index:2147483646!important;width:72px!important;height:72px!important;border-radius:50%!important;border:1px solid rgba(255,255,255,.25)!important;background:rgba(8,8,10,.72)!important;color:#fff!important;font-size:30px!important;display:flex!important;align-items:center!important;justify-content:center!important;box-shadow:0 8px 28px rgba(0,0,0,.35)!important;}" +
        ".xv-play.xv-playing{opacity:.18!important;}" +
        "#xv-controls{position:fixed;left:12px;right:12px;bottom:12px;z-index:2147483647;padding:9px 10px 8px;background:rgba(12,12,14,.82);border:1px solid rgba(255,255,255,.12);border-radius:18px;box-sizing:border-box;font-family:sans-serif;}" +
        ...
        "#xv-actions{position:fixed;right:16px;bottom:78px;z-index:2147483647;display:flex;gap:8px;}#xv-actions button{height:42px;min-width:48px;padding:0 13px;border:1px solid rgba(255,255,255,.14);border-radius:21px;background:rgba(12,12,14,.82);color:#fff;font-size:14px;font-weight:700;}" +
        "`;document.head.appendChild(st);" +
        "function fmt(s){if(!isFinite(s)||s<0)s=0;var m=Math.floor(s/60),q=Math.floor(s%60);return m+':'+(q<10?'0':'')+q;}" +
        "function btnFor(a){return a&&a.querySelector('.xv-play');}" +
        "function syncBtn(v,a){var b=btnFor(a);if(!b)return;if(v.paused){b.textContent='▶';b.classList.remove('xv-playing');}else{b.textContent='❚❚';b.classList.add('xv-playing');}}" +
        ...
        "function prepVideo(v,priority){if(!v)return;v.controls=false;v.autoplay=false;v.playsInline=true;v.setAttribute('playsinline','');v.preload=priority?'auto':'metadata';}" +
        "function addPlayButton(a,v){if(a.querySelector('.xv-play'))return;var b=document.createElement('button');b.className='xv-play';b.textContent='▶';b.setAttribute('aria-label','播放/暂停');b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();setActive(v,a);if(v.paused){document.querySelectorAll('video').forEach(function(o){if(o!==v&&!o.paused){o.pause();syncBtn(o,o.closest('article[data-testid=\\\"tweet\\\"]'));}});v.playbackRate=currentSpeed;v.play().catch(function(){});}else{v.pause();}syncBtn(v,a);};a.appendChild(b);v.addEventListener('play',function(){syncBtn(v,a);});v.addEventListener('pause',function(){syncBtn(v,a);});}" +
        "function normalizeArticle(a){var v=a.querySelector('video');if(!v)return false;a.style.setProperty('display','block','important');a.dataset.xvBound='1';a.dataset.xvNorm='1';var r=a.getBoundingClientRect();a.style.transform='translateX('+(-r.left)+'px) translateZ(0)';a.style.width=innerWidth+'px';var n=v;while(n&&n!==a){n.style.setProperty('position','static','important');n.style.setProperty('width','100%','important');n.style.setProperty('height','100%','important');n.style.setProperty('max-width','none','important');n.style.setProperty('max-height','none','important');n.style.setProperty('margin','0','important');n.style.setProperty('padding','0','important');n=n.parentElement;}function prune(p){Array.from(p.children).forEach(function(c){if(c.classList&&c.classList.contains('xv-play'))return;if(c===v||c.contains(v)){c.style.setProperty('display','block','important');if(c!==v)prune(c);}else c.style.setProperty('display','none','important');});}prune(a);prepVideo(v,false);v.pause();addPlayButton(a,v);return true;}" +
        "function setupScroller(){if(window.__xvScroller)return;var a=document.querySelector('article[data-testid=\\\"tweet\\\"][data-xv-bound=\\\"1\\\"]');if(!a)return;var p=a.parentElement;while(p&&p!==document.body){var cs=getComputedStyle(p);if((cs.overflowY==='auto'||cs.overflowY==='scroll')&&p.scrollHeight>p.clientHeight){p.style.scrollSnapType='y mandatory';p.style.overscrollBehavior='none';window.__xvScroller=p;return;}p=p.parentElement;}document.documentElement.style.scrollSnapType='y mandatory';window.__xvScroller=document.documentElement;}" +
        "var io=new IntersectionObserver(function(es){es.forEach(function(e){var a=e.target,v=a.querySelector('video');if(!v)return;if(e.isIntersecting)prepVideo(v,e.intersectionRatio>.05);if(e.isIntersecting&&e.intersectionRatio>=.55)setActive(v,a);if(e.intersectionRatio<.2&&!v.paused){v.pause();syncBtn(v,a);}});},{threshold:[0,.05,.2,.55,.85]});" +
        "var nearIo=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){var v=e.target.querySelector('video');if(v)prepVideo(v,true);}});},{rootMargin:'120% 0px 120% 0px',threshold:0});" +
        "function bindVideoArticle(a){if(a.dataset.xvBound==='1')return;var v=a.querySelector('video');if(!v)return;normalizeArticle(a);io.observe(a);nearIo.observe(a);}" +
        "function deferHide(a){if(a.dataset.xvHidePending==='1'||a.dataset.xvBound==='1')return;a.dataset.xvHidePending='1';setTimeout(function(){a.dataset.xvHidePending='0';if(a.querySelector('video')){bindVideoArticle(a);return;}if(a.isConnected)a.style.setProperty('display','none','important');},2500);}" +
        "function processNew(){rafPending=false;document.querySelectorAll('article[data-testid=\\\"tweet\\\"]').forEach(function(a){if(a.querySelector('video')){a.style.setProperty('display','block','important');bindVideoArticle(a);}else if(a.dataset.xvBound!=='1'){deferHide(a);}});setupScroller();}" +
